...
route.get('/', verifyToken, (req, res, next)=>{
    ...
    let data_rows;
    model_study.listStudy({search: search, orderby: sortby, order: sort, index: index, len: rows}, (err, result, field)=>{
        if(err){
            return res.status(500).send({response: null, message: err.message});
        }
        data_rows = result;
        model_study.getAllRows(search, (err, result, field)=>{
            if(err){
                return res.status(500).send({response: null, message: err.message});
            }
...
route.get('/light', verifyToken, (req, res)=>{
    let search = req.query.search;
    let data_rows = [];
    model_study.listLightStudy({search: search}, (err, result, field)=>{
        if(err){
            data_rows = [];
        }else{
...
route.get('/:id', verifyToken, (req, res, next)=>{
    ...
    model_study.getStudy({id: id}, (err, result, field)=>{
        if(err){
            return res.status(500).send({data: false, message: err.message});
        }
        if(result.length > 0){
